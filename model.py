import math
import numpy as np
from numba.cuda import lanemask_lt
import torch
...
class PGM(nn.Module):
    ...
    def _project(self, grads):
        ...
        alpha = torch.from_numpy(res.x).float().cuda()
        return alpha
...
class SPFusion(nn.Module):
    def __init__(self, input_dim, rank=16, poly_order=4,output_dim=1024):
        super().__init__()
        self.rank = rank
        self.output_dim = output_dim
        self.input_dim = input_dim
        self.poly_order = poly_order
        self.weight_list_text = nn.ParameterList([
            nn.Parameter(torch.Tensor(input_dim, rank)) for _ in range(poly_order)
        ])
        self.weight_list_audio = nn.ParameterList([
            nn.Parameter(torch.Tensor(input_dim, rank)) for _ in range(poly_order)
        ])
        self.weight_list_vision = nn.ParameterList([
            nn.Parameter(torch.Tensor(input_dim, rank)) for _ in range(poly_order)
        ])
        self.weight_list_const = nn.ParameterList([
            nn.Parameter(torch.Tensor(1, rank)) for _ in range(poly_order)
        ])


        self.modality_gates = nn.ParameterList([
            nn.Parameter(torch.ones(3)) for _ in range(self.poly_order)
        ])
        self.gate_act = nn.Sigmoid()
        self.act = gelu

        for wl in [self.weight_list_text, self.weight_list_audio, self.weight_list_vision, self.weight_list_const]:
            for w in wl:
                nn.init.xavier_normal_(w)

        self.output_proj = nn.Parameter(torch.Tensor(rank, output_dim))
        nn.init.xavier_normal_(self.output_proj)

        self.output_bias = nn.Parameter(torch.zeros(output_dim))
        self.residual_proj = nn.Linear(input_dim * 3, output_dim)
        self.eps = 1e-6

    def forward(self, t, a, v):
        x = torch.cat([t, a, v], dim=-1)  # [B, L, 3D]
        B, L, D3 = x.shape
        ...
        for i in range(self.poly_order):
            ...
            # [B, L, R] = [B, L, D] @ [D, R]
            z_t = self.act(torch.matmul(t, self.weight_list_text[i]))
            z_a = self.act(torch.matmul(a, self.weight_list_audio[i]))
            z_v = self.act(torch.matmul(v, self.weight_list_vision[i]))

            ones = torch.ones(B, L, 1, device=t.device)
            z_c = torch.matmul(ones, self.weight_list_const[i])  # [B, L, 1] × [1, R] → [B, L, R]

            gates = self.gate_act(self.modality_gates[i])  # [3], each ∈ (0,1)
            z_p = gates[0] * z_t + gates[1] * z_a + gates[2] * z_v + z_c

            out = out * z_p


        out = torch.sign(out) * torch.sqrt(torch.abs(out) + self.eps)
        fused = torch.matmul(out, self.output_proj) + self.output_bias  # [B, L, O]

        residual = self.residual_proj(x)  # [B, L, O]
...
class Transformer_Based_Model(nn.Module):
    def __init__(self, dataset, temp, D_text, D_visual, D_audio, n_head,rank,poly_order,
        ...
        self.last_gate = SPFusion(hidden_dim, rank, poly_order, output_dim=hidden_dim)
    ...
    def forward(self, textf, visuf, acouf, u_mask, qmask, dia_len):
        ...
        if self.dataset == 'MELD':
            ...
            proj_audio_bias = self.text_to_audio(text_bias)  # [B, L, D]
            gate_audio = torch.sigmoid(self.bias_gate_audio(text_bias))  # [B, L, D]
